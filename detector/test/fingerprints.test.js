...
test('ai-placeholder fires on common slot-fill bracket patterns', () => {
  ...
  for (const text of [
    ...
    const r = AIDetector.analyzeText(
      text + ' Additional padding text to clear the word-count gate. '.repeat(2),
    );
...
test('ai-placeholder does not fire on legitimate bracketed content', () => {
  ...
  const text =
    'The release notes for [v1.2.3] cover the [auth.refresh] path and reference [@example/user]. We saw it on commit [a3f7b21]. Padding text to clear the word-count gate so the analyzer runs the full pass cleanly.';
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    !types.has('ai-placeholder'),
    `expected no ai-placeholder, got: ${[...types].join(', ')}`,
  );
});
...
test('ai-citation-markup fires on chatbot-internal tokens', () => {
  ...
  for (const text of [
    ...
    'The grok_card here links to the relevant policy. ' +
      'Padding text to clear the gate. '.repeat(3),
  ]) {
    const r = AIDetector.analyzeText(
      text + ' '.repeat(0) + 'Padding text to clear the word-count gate. '.repeat(2),
    );
    const types = new Set(r.issues.map((i) => i.type));
    assert.ok(
      types.has('ai-citation-markup'),
      `expected ai-citation-markup for: ${text.slice(0, 50)}...`,
    );
  }
});
...
test('ai-utm-source fires on AI-tool tracking parameters', () => {
  ...
  for (const text of [
    ...
    const r = AIDetector.analyzeText(
      text +
        ' Padding text to clear the word-count gate so the analyzer runs cleanly across all categories.',
    );
    const types = new Set(r.issues.map((i) => i.type));
...
test('ai-utm-source does not fire on benign utm_source values', () => {
  ...
  const text =
    'See https://example.com/article?utm_source=newsletter for the source. Padding text to clear the word-count gate so the analyzer runs the full pass cleanly across all categories without surprises.';
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
