...
- **`code`** — source files, diffs, commit messages, PR descriptions, tests. Turns on the code-and-repository-artifacts patterns (#81-88) and turns off prose-only rules that do not apply to code: sentence-length rhythm, copula avoidance, rule of three, transition phrases, paragraph-length uniformity, and TTR stylometry. Em-dash and inflated-vocabulary rules still apply, but only to prose surfaces (comments, commit bodies, PR text), not to identifiers or string literals. The two logic-touching patterns (#85, #87) are flag-only here: report and propose, never auto-cut.
...
| Tier 3 phrase clustering | relaxed | strict | relaxed | strict | relaxed | skip |
| Future-narrative closers | relaxed | strict | strict | extra strict | skip | skip |
| Moral-adjective category errors | strict | strict | strict | strict | strict | relaxed |
| Invented contrast-pair mirroring | strict | strict | strict | strict | relaxed | skip |
| Subjectless fragments and agentless passives | relaxed | strict | relaxed | strict | skip (fragments are the form) | skip |
| Real/actual inflation | strict | strict | strict | strict | relaxed | skip |
| Launch-copy introductions | relaxed (1 hook) | strict | strict | strict | strict | skip |
| Unnecessary hyphenation | relaxed | strict | strict | strict | strict | skip |
