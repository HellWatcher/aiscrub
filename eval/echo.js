...
const fixtures = JSON.parse(fs.readFileSync(path.join(__dirname, 'echo-fixtures.json'), 'utf8'));
...
const sweep = [12, 15, 18, 20, 25].map((w) => ({
  window: w,
  ...evalMode({ echoStem: false, echoWindow: w }),
}));
...
if (asJson) {
  console.log(
    JSON.stringify(
      { n: fixtures.length, defaultWindow: DEFAULT_WINDOW, exact, stem, sweep },
      null,
      2,
    ),
  );
  ...
  console.log(
    `exact  (window ${DEFAULT_WINDOW})  precision ${pct(exact.precision)}  recall ${pct(exact.recall)}  ` +
      `FP-rate ${pct(exact.fpRate)}  (tp${exact.tp} fp${exact.fp} tn${exact.tn} fn${exact.fn})`,
  );
  console.log(
    `stem   (window ${DEFAULT_WINDOW})  precision ${pct(stem.precision)}  recall ${pct(stem.recall)}  ` +
      `FP-rate ${pct(stem.fpRate)}  (tp${stem.tp} fp${stem.fp} tn${stem.tn} fn${stem.fn})`,
  );
  console.log('\nexact-mode window sweep:');
  for (const s of sweep) {
    console.log(
      `  window ${String(s.window).padEnd(3)} precision ${pct(s.precision)}  recall ${pct(s.recall)}  FP-rate ${pct(s.fpRate)}`,
    );
  }
