...
const STOCK = new Set([
  "landscape", "tapestry", "realm", "beacon", "ecosystem", "navigate",
  "navigating", "journey", "unlock", "unlocks", "unleash", "symphony",
  "illuminate", "illuminating", "cornerstone", "catalyst", "springboard",
  "roadmap", "lighthouse", "tide", "wave", "engine", "fuels", "bridge",
  "weaving", "weave", "threads", "thread", "vision", "potential",
]);

// ── Approach B lists ──
const IMAGE_NOUNS = new Set([
  "lens", "thread", "plot", "path", "road", "bridge", "anchor", "mirror",
  "window", "door", "wall", "mountain", "ocean", "current", "tide", "root",
  "seed", "engine", "gear", "compass", "map", "lighthouse", "fog", "storm",
  "knot", "web", "fabric", "beacon",
]);
const PHYSICAL_VERBS = new Set([
  "dig", "lift", "carry", "weave", "wrestle", "grab", "grasp", "plant",
  "anchor", "bridge", "peel", "untangle", "unravel", "chase", "hunt",
  "climb", "knit", "stitch", "juggle", "steer", "sail", "swim", "drown",
  "sink", "float", "navigate", "work",
]);
const ABSTRACT_NOUNS = new Set([
  "problem", "impact", "value", "idea", "decision", "strategy", "growth",
  "truth", "meaning", "risk", "work", "career", "conversation", "vision",
  "innovation", "collaboration", "opportunity", "potential", "future",
]);
...
function novelHits(text) {
  ...
  for (let i = 0; i < t.length - 2; i++) {
    if ((t[i] === "is" || t[i] === "are") && t[i + 1] === "the" && IMAGE_NOUNS.has(t[i + 2])) {
      hits++; reasons.push(`copular:${t[i + 2]}`);
    }
  }
  ...
  for (let i = 0; i < t.length - 2; i++) {
    if (PHYSICAL_VERBS.has(t[i]) && t[i + 1] === "the" && ABSTRACT_NOUNS.has(t[i + 2])) {
      hits++; reasons.push(`physabs:${t[i]}>${t[i + 2]}`);
    }
  }
  // (3) frozen image idioms
  for (const re of IMAGE_IDIOMS) {
    if (re.test(text)) { hits++; reasons.push(`idiom:${re.source.slice(0, 18)}`); }
  }
...
function confusion(predict) {
  ...
  for (const s of scored) {
    ...
    else if (s.figurative && !pred) { fn++; fns.push(s.id); }
    else if (!s.figurative && pred) { fp++; fps.push(s.id); }
    else tn++;
  }
...
const report = {
  ...
  densityByClass: Object.fromEntries(
    Object.entries(classes).map(([k, fn]) => [k, {
      stockPer100: +meanBy(fn, (s) => s.stockScore.per100).toFixed(2),
      novelPer100: +meanBy(fn, (s) => s.novelScore.per100).toFixed(2),
    }])
  ),
  approachA_stock: { precision: aClassifier.precision, recall: aClassifier.recall, fp: aClassifier.fps, fn: aClassifier.fns },
  approachB_novel: { precision: bClassifier.precision, recall: bClassifier.recall, fp: bClassifier.fps, fn: bClassifier.fns },
...
if (asJson) {
  ...
  for (const [k, v] of Object.entries(report.densityByClass)) {
    console.log(`  ${k.padEnd(18)} ${String(v.stockPer100).padStart(6)}  ${String(v.novelPer100).padStart(6)}`);
  }
  console.log("\nfigurative-sentence detection:");
  console.log(`  Approach A (stock lexicon)   precision ${pct(aClassifier.precision)}  recall ${pct(aClassifier.recall)}` +
    `  FP[${aClassifier.fps.join(",") || "-"}]`);
  console.log(`  Approach B (novel heuristic) precision ${pct(bClassifier.precision)}  recall ${pct(bClassifier.recall)}` +
  ...
  if (bClassifier.fps.length) {
    ...
    bClassifier.fps.forEach((id) => {
      ...
      console.log(`  ${id} [${s.source}]: ${s.novelScore.reasons.join(", ")}  — "${s.text.slice(0, 60)}..."`);
    });
  }
}
